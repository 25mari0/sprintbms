import {
    Table, TableBody, TableCell, TableHead, TableRow, Button, Tooltip, Chip, Fade, Box, Typography
  } from '@mui/material';
      ...
      <Fade in={true}>
        <Table sx={{ backgroundColor: '#1E1E1E', borderRadius: '8px' }}>
          ...
          <TableBody>
            {workers.length > 0 ? (
              workers.map((worker) => (
                <TableRow key={worker.user.id} sx={{ '&:hover': { backgroundColor: '#2A2A2A' } }}>
                ...
                <TableCell colSpan={4} align="center">
                  <Fade in>
                    <Box sx={{ py: 4, color: '#A8C7E2' }}>
                      ...
                      <Button variant="contained" startIcon={<Add />} color="primary" onClick={onCreateClick} sx={{textAlign: 'center', borderRadius: '20px' }}>
                      Create Worker
                      </Button>
                    </Box>
                  </Fade>
